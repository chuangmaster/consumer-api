...
public interface IPreferenceRepository
{
    ...
    Task<MfUserPreference> CreateAsync(string userId, string[] enabledWidgets, LayoutItemDto[] layoutData);
    /// <summary>
    /// 使用樂觀鎖更新偏好設定。
    /// </summary>
    /// <returns>true 代表更新成功；false 代表版本不符（已被其他請求修改），應回傳 409</returns>
    Task<bool> UpdateAsync(string userId, string[] enabledWidgets, LayoutItemDto[] layoutData, int expectedVersion);
}
